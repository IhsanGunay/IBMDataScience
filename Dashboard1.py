...
def compute_info(airline_data, entered_year):
    ...
    avg_car = df.groupby(['Month', 'Reporting_Airline'])['CarrierDelay'] \
        .mean().reset_index()
    avg_weather = df.groupby(['Month', 'Reporting_Airline'])['WeatherDelay'] \
        .mean().reset_index()
    avg_NAS = df.groupby(['Month', 'Reporting_Airline'])['NASDelay'] \
        .mean().reset_index()
    avg_sec = df.groupby(['Month', 'Reporting_Airline'])['SecurityDelay'] \
        .mean().reset_index()
    avg_late = df.groupby(['Month', 'Reporting_Airline'])['LateAircraftDelay']\
        .mean().reset_index()
...
def get_graph(entered_year):
    ...
    carrier_fig = px.line(avg_car, x='Month', y='CarrierDelay',
                          color='Reporting_Airline', title='Avg Carrier Delay')
    weather_fig = px.line(avg_weather, x='Month', y='WeatherDelay',
                          color='Reporting_Airline', title='Avg Weather Delay')
    nas_fig = px.line(avg_nas, x='Month', y='NASDelay',
                      color='Reporting_Airline', title='NAS Delay')
    sec_fig = px.line(avg_sec, x='Month', y='SecurityDelay',
                      color='Reporting_Airline', title='Security Delay')
    late_fig = px.line(avg_late, x='Month', y='LateAircraftDelay',
                       color='Reporting_Airline', title='Late Aircraft Delay')
